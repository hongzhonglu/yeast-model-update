...
panGenome <- filter(panGenome, pident >= pident0 & bitscore0 >=100)
...
s288Genome <- filter(s288Genome, pident >= pident0 & bitscore0 >=100)
...
#second method to establish the gene ec reactions relation
...
newEC <- setdiff(panGene0_ec$EC,ec_s288c0$EC)
...
metanetx_rxn <- read_tsv("/home/hongzhong/hongzhong/panGenome and coreGenome/reac_prop_metanetx.tsv")
...
panGene0_newEC$MNXID <- getMultipleReactionFormula(metanetx_rxn1$v2,metanetx_rxn1$v1,panGene0_newEC$EC)
panGene0_newEC <- panGene0_newEC[which(!is.na(panGene0_newEC$MNXID)),]


panGene_rxn <- splitAndCombine(panGene0_newEC$MNXID,panGene0_newEC$panID,sep0 = ";")
panGene_rxn$EC <- getMultipleReactionFormula(panGene0_newEC$EC,panGene0_newEC$panID,panGene_rxn$v2)
...
write.table(panGene_rxn0,'new rxn from pan based on uniprot.txt', row.names = FALSE, sep = "\t")
write.table(panGene0_newEC,'new ec from pan based on uniprot.txt', row.names = FALSE, sep = "\t")
