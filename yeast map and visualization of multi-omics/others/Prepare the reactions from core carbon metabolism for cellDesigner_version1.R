...
currency_metabolites <- c("H+", "H2O", "ATP", "phosphate", "coenzyme A", "ADP", "diphosphate", "NADP(+)", "NADPH", "carbon dioxide", "AMP","NAD", "CMP", "NADH") #"oxygen","acetyl-CoA","hydrogen peroxide")     
...
index_currency<- which(rxn_split_refine$simple_name %in% currency_metabolites == TRUE)
...
index_combine <- which(str_detect(rxn_split_refine$subsystem, "glycerolipid metabolism \\( sce00561 \\)")==TRUE)
...
x_vector <- seq(100, 10000, by=100)
y_vector <- seq(100, 10000, by=100)
