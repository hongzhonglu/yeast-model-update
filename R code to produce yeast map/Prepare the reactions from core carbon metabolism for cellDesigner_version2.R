...
# choose reactions based on subsytem
subsystem1 <- "glycolysis"
subsystem2 <- "citrate cycle"
subsystem3 <- "pentose phosphate pathway"
choosed_subsystem <- c(subsystem1, subsystem2, subsystem3)


choosed_subsystem <- "glycerophospholipid metabolism \\( sce00564 \\)"



## define the currency in specific subsystem

index_combine <- which(str_detect(metabolite_withoutCompartment$subsystem, choosed_subsystem)==TRUE)
metabolite_subsystem <- metabolite_withoutCompartment[index_combine,]
metabote_analysis_subsystem <- metabolite_subsystem %>%
  count(name_simple) %>%
  arrange(.,desc(n))
currency_metabolites_from_subsystem <- metabote_analysis_subsystem$name_simple[1:106]
...
index_currency<- which(rxn_split_refine$simple_name %in% currency_metabolites_general == TRUE) ### it should be noted: currency_metabolites should be small range
...
index_combine <- which(str_detect(rxn_split_refine$subsystem, choosed_subsystem)==TRUE)
